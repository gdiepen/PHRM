...
function loadRangeData(){
	...
	var range_percentiles_low = []
	var range_percentiles_high = []
	for(var i=NUMBER_OF_BANDS-1 ; i>=0 ; i--){
		range_percentiles_low.push( 0.5 - ((i+1.0)/NUMBER_OF_BANDS)/2.0)
		range_percentiles_high.push( 0.5 + ((NUMBER_OF_BANDS-1-i+1.0)/NUMBER_OF_BANDS)/2.0)
	}

	store.settings.range_percentiles = range_percentiles_low.concat(range_percentiles_high) ;
	console.log(store.settings.range_percentiles)
	...
	Object.entries(store.settings).forEach(([key, value]) => {
		if (Array.isArray(value)){
			...
			value.forEach(arr_value => {
				searchParams.append(key, arr_value.toFixed(4)) ;
			});
		}
		else{
...
function showRangeData(){
	...
	var color = d3.scaleSequential(d3.interpolatePlasma)
		.domain([-NUMBER_OF_BANDS/6, NUMBER_OF_BANDS])
	...
	for(var i=0 ;i<NUMBER_OF_BANDS ; i++){
		...
		range_points.enter()
			...
			.attr("d", d3.area()
				.x(function (d) {return store.x(d.hours_since_midnight)})
				.y0(function (d) {return store.y(d[i])})
				.y1(function (d) {return store.y(d[2*NUMBER_OF_BANDS-1-i])})
			)
